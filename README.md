...
This is a fork of the [WoWSims Classic](https://github.com/wowsims/classic) simulator, adapted for Turtle WoW.
...
[Live sims can be found here.](https://isfir.github.io/wowsims-turtle 'https://isfir.github.io/wowsims-turtle')
...
-   [Windows Sim](https://github.com/isfir/wowsims-turtle/releases/latest/download/wowsims-turtle-windows.exe.zip)
-   [MacOS Sim](https://github.com/isfir/wowsims-turtle/releases/latest/download/wowsims-turtle-amd64-darwin.zip)
-   [Linux Sim](https://github.com/isfir/wowsims-turtle/releases/latest/download/wowsims-turtle-amd64-linux.zip)

Then unzip the downloaded file, then open the unzipped file to open the sim in your browser!

Alternatively, you can choose from a specific relase on the [Releases](https://github.com/isfir/wowsims-turtle/releases) page and click the suitable link under "Assets"
...
docker build --tag wowsims-turtle .
docker run --rm -v $(pwd):/app wowsims-turtle npm install

# Now you can run the commands as shown in the Commands sections, preceding everything with, "docker run --rm -it -p 8080:8080 -v $(pwd):/app wowsims-turtle".
# For convenience, set this as an environment variable:
WOWSIMS_CMD="docker run --rm -it -p 8080:8080 -v $(pwd):/app wowsims-turtle"

#For the watch commands assign this environment variable:
WOWSIMS_WATCH_CMD="docker run --rm -it -p 8080:8080 -p 3333:3333 -p 5173:5173 -e WATCH=1 -v $(pwd):/app wowsims-turtle"

# ... do some coding on the sim ...

# Run tests
$(echo $WOWSIMS_CMD) make test

# ... do some coding on the UI ...

# Host a local site
$(echo $WOWSIMS_CMD) make host
```
...
# http://localhost:8080/YOUR_SPEC_HERE, where YOUR_SPEC_HERE is the directory under ui/ with your custom code.
# Recompiles the entire client before launching using `make dist`
make host
...
# Recompiles the `wowsims-turtle` server binary and runs it, hosting /dist directory at http://localhost:3333.
...
# The same as rundevserver, recompiles  `wowsims-turtle` binary and runs it on port 3333. Instead of serving content from the dist folder,
...
# Creates the 'wowsims-turtle' binary that can host the UI and run simulations natively (instead of with wasm).
# Builds the UI and the compiles it into the binary so that you can host the sim as a server instead of wasm on the client.
# It does this by first doing make dist and then copying all those files to binary_dist/ and loading all the files in that directory into its binary on compile.
make wowsims-turtle
...
# make dist && ./wowsims-turtle --usefs would rebuild the whole client and host it. (you would have had to run `make devserver` to build the wowsims-turtle binary first.)
./wowsims-turtle --usefs
...
When you're ready to try out the site, run `make host` and navigate to `http://localhost:8080/$SPEC`.
